    ...
    Scaffold(
        ...
        Column(
            ...
            when {
                ...
                mono != null -> {
                    val preview = remember(mono) { MonoConverter.toBitmap(mono!!).asImageBitmap() }
                    Image(
                        bitmap = preview,
                        contentDescription = null,
                        modifier = Modifier.fillMaxWidth().heightIn(min = 180.dp, max = 520.dp).border(1.dp, MaterialTheme.colorScheme.outlineVariant),
                        contentScale = ContentScale.Fit,
                        filterQuality = FilterQuality.None
                    )
                    Spacer(Modifier.height(6.dp))
                    Text("384 点宽 · 预计长度约 ${mono!!.height / 8} mm", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
