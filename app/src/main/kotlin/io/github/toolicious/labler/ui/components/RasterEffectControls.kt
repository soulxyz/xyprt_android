...
/**
 * One compact raster/effect control used everywhere an image is prepared for printing.
 * Quick image, PDF and the normal editor intentionally share this UI so the same option
 * always has the same Chinese name and layout.
 */
@Composable
fun RasterEffectControls(
    ...
    Row(
        ...
        Text("打印效果", style = MaterialTheme.typography.labelLarge, modifier = Modifier.width(62.dp), maxLines = 1)
        listOf(
    ...
    if (rotationDegrees != null && onRotationDegrees != null) {
        Row(
            ...
            Text("旋转", style = MaterialTheme.typography.labelLarge, modifier = Modifier.width(62.dp))
...
private fun CompactSliderLine(
    ...
    Row(
        ...
        Text(label, style = MaterialTheme.typography.bodyMedium, modifier = Modifier.width(84.dp), maxLines = 1)
