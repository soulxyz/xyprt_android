 ...
 * FF02 is written WITH response when available. Android intentionally stays on the default ATT MTU
 * and starts at 20-byte payloads; status 13 dynamically falls back to 16/12/8 bytes.
 */
...
class BlePrinterConnection private constructor(
    ...
    override var chunkSize: Int = Protocol.BLE_INITIAL_CHUNK_SIZE
        private set
    ...
    private suspend fun attach() = withContext(Dispatchers.IO) {
        ...
        // Do not call requestMtu(). Some Qring bridges accept a large negotiated MTU but reject
        // characteristic writes at the Android GATT layer. Start at the default-ATT-safe 20 bytes.
        chunkSize = Protocol.BLE_INITIAL_CHUNK_SIZE
        log("BLE ready: ${device.address}, chunk=$chunkSize, response=$writeWithResponse")
    }
    ...
    override suspend fun write(bytes: ByteArray) {
        ...
        while (offset < bytes.size) {
            ...
            } catch (t: GattWriteStatusException) {
                if (t.status != BluetoothGatt.GATT_INVALID_ATTRIBUTE_LENGTH || chunkSize <= Protocol.BLE_MIN_CHUNK_SIZE) throw t
                val oldSize = chunkSize
                chunkSize = Protocol.nextBleChunkSize(oldSize)
                log("BLE ${oldSize}B write rejected (13); retrying same data at ${chunkSize}B")
                delay(120)
            }
        }
    }
        ...
        repeat(MAX_WRITE_RETRY) { attempt ->
            try {
                if (writeWithResponse) {
                    ...
                    if (status != BluetoothGatt.GATT_SUCCESS) throw GattWriteStatusException(status)
                } else {
                    val started = if (Build.VERSION.SDK_INT >= 33) {
                        g.writeCharacteristic(
                            writeCharacteristic,
                ...
                if (t is GattWriteStatusException && t.status == BluetoothGatt.GATT_INVALID_ATTRIBUTE_LENGTH) throw t
                last = t
    ...
    private class GattWriteStatusException(val status: Int) : IllegalStateException(
        when (status) {
            BluetoothGatt.GATT_INVALID_ATTRIBUTE_LENGTH -> "BLE 写入失败（13）：分包长度不兼容"
            BluetoothGatt.GATT_INSUFFICIENT_AUTHENTICATION -> "BLE 需要先完成配对（5）"
            BluetoothGatt.GATT_INSUFFICIENT_ENCRYPTION -> "BLE 需要加密/配对（15）"
            else -> "BLE 写入失败（$status）"
        }
    )
